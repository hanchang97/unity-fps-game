...
public class BossEnemyFSM : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        switch (m_State)
        {
            ...
            case BossEnemyState.Attack:
                Attack();
                break;
            case BossEnemyState.Return:
                Return();
                break;
            case BossEnemyState.Damaged:    // 피격 상태, 사망 상태 함수는 매 프레임마다 반복 실행이 아니라 1회만 실행되어야 하므로
                //Damaged();
                break;
    ...
    public void HitEnemy(int hitPower)
    {
        ...
        if (hp > 0)
        {
            m_State = BossEnemyState.Damaged;
